    ...
    constructor(config) {
        ...
        this.pullLogs = (option, selector) => this.noBodyAction({
            ...
            template: `http://{projectName}.{endpoint}/logstores/{logStore}/shards/${option.shards ||
                0}`
        }, { cursor: option.cursor, count: option.count || 10, type: 'log' }, selector).pipe(operators_1.pluck('response'), operators_1.map((buffer) => sls_1.sls.LogGroupList.decode(new Uint8Array(buffer, 0, buffer.byteLength))));
        /**
...
SlsClient.queryString = (queries) => Object.entries(queries)
    ...
    .sort()
    .join('&');
SlsClient.createKvList = (obj) => Object.entries(obj).map(([Key, Value]) => ({ Key, Value }));
