...
class SlsClient {
    constructor(config) {
        ...
        this.noBodyAction = (urlParams, queries, selector) => {
            ...
            return this.action(url + '?' + SlsClient.queryString(queries), 'GET', queries);
        };
        /**
         * @description 拉取日志
         * @memberof SlsClient
        ...
        this.pullLogs = (option, selector) => this.noBodyAction({
            ...
            template: `http://{projectName}.{endpoint}/logstores/{logStore}/shards/${option.shards ||
                0}`
        }, { cursor: option.cursor, count: option.count || 10, type: 'log' }, selector).pipe(operators_1.pluck('response'), operators_1.map((buffer) => LogGroupListProto.decode(new Uint8Array(buffer, 0, buffer.byteLength))));
        /**
         * @description 指定游标获取日志
         * @memberof SlsClient
         */
        this.getLogs = (option, selector) => {
...
SlsClient.bufferToLogList = (buffer) => LogGroupListProto.decode(buffer);
SlsClient.getCanonicalizedHeaders = (headers) => Object.keys(headers)
    ...
    .join('\n') + '\n';
SlsClient.getCanonicalizedResource = (path, queries) => `${path}?${SlsClient.queryString(queries)}`;
